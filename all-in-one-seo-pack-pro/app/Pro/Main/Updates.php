...
class Updates extends CommonMain\Updates {
	...
	public function runUpdates() {
		...
		if ( version_compare( $lastActiveVersion, '4.0.0', '>=' ) && version_compare( $lastActiveVersion, '4.0.5', '<' ) ) {
			try {
				set_transient( 'aioseo_v4_migrate_post_og_image', time(), WEEK_IN_SECONDS );
				set_transient( 'aioseo_v4_migrate_term_og_image', time(), WEEK_IN_SECONDS );
	...
	public function migratePostOgImage() {
		...
		if ( get_transient( 'aioseo_v3_migration_in_progress_posts' ) ) {
			try {
		...
		$timeStarted     = gmdate( 'Y-m-d H:i:s', get_transient( 'aioseo_v4_migrate_post_og_image' ) );
		...
		if ( ! $postsToMigrate || ! count( $postsToMigrate ) ) {
			delete_transient( 'aioseo_v4_migrate_post_og_image' );
			return;
		}
	...
	public function migrateTermOgImage() {
		// If the v3 migration is still running, postpone this.
		if ( get_transient( 'aioseo_v3_migration_in_progress_terms' ) ) {
			try {
		...
		$timeStarted      = gmdate( 'Y-m-d H:i:s', get_transient( 'aioseo_v4_migrate_term_og_image' ) );
		...
		if ( ! $termsToMigrate || ! count( $termsToMigrate ) ) {
			delete_transient( 'aioseo_v4_migrate_term_og_image' );
			return;
		}
